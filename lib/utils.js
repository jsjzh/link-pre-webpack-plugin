...
function addOutput(compilation, filename, content) {
  compilation.assets[filename] = _createAsset(content)
}
...
function isInHtmlPluginBehind(compiler) {
  ...
  let htmlIndex = plugins.reduce(
    (prev, curr, currIndex) =>
      supportHtmlTemplatePlugins.includes(_constructorName(curr)) ? [...prev, currIndex] : prev,
    []
  )
  return pluginIndex === Math.max.apply(null, [...htmlIndex, pluginIndex])
}
