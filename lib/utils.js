...
function transChunksToHrefs(compilation, chunks, fileType) {
  let {
    assets,
    options: {
      output: { publicPath = '' }
  ...
  chunks.forEach(chunk => {
    if (judgeType(chunk) === '[object String]') chunk = { href: chunk, attrs: [] }
...
function transHrefsToPares5Attrs(hrefs, preAs = 'script', preType = 'preload') {
